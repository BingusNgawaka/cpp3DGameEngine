#include "../../include/core/TextureLoader.hpp"
...
GLuint LoadTextureFromMTL(const std::string& mtlPath) {
    ...
    std::string mtlFolder = std::filesystem::path(mtlPath).parent_path().string()+"/";
    ...
    while (std::getline(mtlFile, line)) {
        if (line.substr(0, 7) == "map_Kd ") {
            // set texturePath relative to mtl folder
            texturePath = mtlFolder + line.substr(7);
            break;
        }
    }
